...
def draw_restaurant_card(res):
    with st.container(border=True):
        ...
        # 4) ✨네이버 지도 연동 링크 수정 (지역명 + 가게이름 결합)
        search_query = f"{res['region']} {res['name']}"
...
with st.sidebar:
    ...
    with st.form("add_form", clear_on_submit=True):
        new_name = st.text_input("가게 이름", placeholder="예: 대성갈비")
